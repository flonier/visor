                                                  ...
                                                  const VkDeviceSize *pOffsets)
{
  ...
  {
    auto cmd = commandBuffer->push<cmd::BindVB>();
...
{
  auto cmd = commandBuffer->push<cmd::BindIB>();
  cmd->buffer = buffer;
  cmd->offset = offset;
...
{
  auto cmd = commandBuffer->push<cmd::PushConstants>();
  cmd->offset = offset;
  cmd->size = size;
...
{
  auto cmd = commandBuffer->push<cmd::DrawIndexed>();
                                                  ...
                                                  const VkBufferImageCopy *pRegions)
{
  ...
  {
    auto cmd = commandBuffer->push<cmd::CopyBuf2Img>();
                                           ...
                                           const VkBufferCopy *pRegions)
{
  for(uint32_t r = 0; r < regionCount; r++)
  {
    auto cmd = commandBuffer->push<cmd::CopyBuf>();
    cmd->srcBuffer = srcBuffer;
                                          ...
                                          const VkImageBlit *pRegions, VkFilter filter)
{
  for(uint32_t r = 0; r < regionCount; r++)
  {
    auto cmd = commandBuffer->push<cmd::BlitImage>();
                                          ...
                                          const VkImageCopy *pRegions)
{
  for(uint32_t r = 0; r < regionCount; r++)
  {
    auto cmd = commandBuffer->push<cmd::CopyImage>();
    cmd->srcImage = srcImage;
    cmd->dstImage = dstImage;
    cmd->region = pRegions[r];
  }
